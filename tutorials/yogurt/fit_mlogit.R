#!/usr/bin/env Rscript
# Fit a multinomial logit on the Yogurt dataset (Jain-Vilcassim-Chintagunta 1994)
# and emit coefficients + log-likelihood as JSON on stdout.
#
# Usage:  Rscript fit_mlogit.R <path-to-yogurt.csv>
# Spec:   choice ~ price + feat (alt-specific intercepts; yoplait = reference)
...
if (length(args) < 1) {
    stop("Usage: Rscript fit_mlogit.R <path-to-yogurt.csv>")
}
csv_path <- args[1]
...
data <- dfidx(
    ...
    shape = "wide",
    choice = "choice",
    varying = 2:9,                    # feat.{4 brands} + price.{4 brands}
    sep = ".",
    idnames = c("chid", "alt")
)

model <- mlogit(choice ~ price + feat, data = data, reflevel = "yoplait")
