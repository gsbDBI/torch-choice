#!/usr/bin/env python3
"""Generate synthetic datasets for the paper performance benchmarks."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--output-path",
        type=Path,
        default=Path(__file__).resolve().parent / "synthetic_data",
...
def simulate_choice_data(
    ...
    selected_items = rng.choice(num_items_total, size=num_subsampled_items, replace=False)
    selected_users = rng.choice(NUM_USERS, size=num_subsampled_users, replace=False)
...
def main() -> None:
    args = parse_args()
    experiments: Iterable[str] = ALL_EXPERIMENTS if "all" in args.experiments else set(args.experiments)
    ...
    print(SECTION)
    if "num_records_experiment_small" in experiments:
    ...
    if "full_dataset" in experiments:
        simulate_choice_data(
            train_all=train_all,
            user_latents=user_latents_df,
            item_latents=item_latents_df,
    ...
    if "num_params_experiment_small" in experiments:
        simulate_choice_data(
            train_all=train_all,
            user_latents=user_latents_df,
            item_latents=item_latents_df,
    ...
    if "num_items_experiment_small" in experiments:
        for num_items_sampled in tqdm([10, 20, 30, 50, 100, 150, 200], desc="num_items_experiment_small"):
            simulate_choice_data(
                train_all=train_all,
                user_latents=user_latents_df,
                item_latents=item_latents_df,
    ...
    if "num_items_experiment_large" in experiments:
    ...
    if args.reference_path:
        compare_against_reference(output_path, args.reference_path.resolve())
