...
for (seed in 1:num.seeds) {
  ...
  for (num_items in c(10, 30, 50, 100, 150, 200)) {
    ...
    df <- read.csv(str_glue("./torch_choice_paper_data/simulated_choice_data_num_items_experiment_{num_items}.csv"))
...
write.csv(records, "./results/R_performance_num_items.csv", row.names=FALSE)